...
if (process.env.NODE_ENV != 'test')
{
	(async () => {
		...
		await createRepo(userId, "ssw345-hw4-test");
		await createIssue(userId, "ssw345-hw4", "Test Name", "testing");
		await enableWikiSupport(userId, "ssw345-hw4");
...
async function listBranches(owner,repo)
{
	...
	return new Promise(function(resolve, reject)
	{
		request(options, function (error, response, body) {
			...
			var obj = JSON.parse(body);
			console.log("----------------------------------------------------------------------------");
			console.log("Branches for current Repo:");
			for( var i = 0; i < obj.length; i++ )
			{
				var name = obj[i].name;
				console.log( name );
